...
namespace health_care
{
    public partial class choose : Form
    {
        ...
        {
            SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-ME05R0P;Initial Catalog=health care;Integrated Security=True");
            string quary = "select que_question , que_response  from Questions where que_response != 'NULL'";
            con.Open();
            ...
            while (reader.Read())
            {
                ...
                QuestionsComboBox.Items.Add(reader["que_question"]);
                AnswersComboBox.Items.Add(reader["que_response"]);
            }
            reader.Close();
            con.Close();
        }
        ...
        {
            SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-ME05R0P;Initial Catalog=health care;Integrated Security=True");
            string quary = "select  pat_sub  from patients where pat_sub != 'NULL'";
            con.Open();
            SqlCommand cmd = new SqlCommand(quary, con);
            cmd.CommandType = CommandType.Text;
            SqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
                SUBcomboBox.Items.Add(reader["pat_sub"]);
            }
            reader.Close();
            con.Close();
